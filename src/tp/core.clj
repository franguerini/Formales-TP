...
(declare verificar-parentesis-recursivo)
...
(repl (list 'append 'append 'car 'car 'cdr 'cdr 'cadr 'cadr 'cond 'cond 'cons 'cons 'define 'define
...
(cond
(= fnc '<)            (fnc-menor lae)
(= fnc '+)            (fnc-sumar (fnc-list lae))
(= fnc '-)            (fnc-restar (fnc-list lae))
(= fnc '<)            (fnc-menor (fnc-list lae))
(= fnc '>)            (fnc-mayor (fnc-list lae))
(= fnc '>=)           (fnc-mayor-o-igual (fnc-list lae))
...
(defn fnc-cadr
  ...
  [lae]
  (let [ari (controlar-aridad-fnc lae 1 'cadr), arg1 (first lae)]
  (cond
    (error? ari) ari
    (or (not (seq? arg1)) (empty? arg1)) (generar-mensaje-error :wrong-type-arg1 'cadr arg1)
    :else (first (rest arg1)))))
...
(defn verificar-parentesis-recursivo [caracter cadena contador]
  (cond
    (nil? caracter) contador
    (= '-1 contador) contador
  :else
    (cond
      (= '\( caracter) (verificar-parentesis-recursivo (first cadena) (drop 1 cadena) (+ contador 1))
      (= '\) caracter) (verificar-parentesis-recursivo (first cadena) (drop 1 cadena) (- contador 1))
    :else
      (verificar-parentesis-recursivo (first cadena) (drop 1 cadena) contador)
    )
  )
)
...
(defn verificar-parentesis [cadena]
  (cond
    (empty? cadena) 0
  :else
    (verificar-parentesis-recursivo (first cadena) (drop 1 cadena) '0)
  )
)
...
(defn leer-entrada ([]
  (let [valor-leido (read-line)] (
    cond
      (<= 1 (verificar-parentesis valor-leido)) (leer-entrada valor-leido)
      (= 0 (verificar-parentesis valor-leido)) (str valor-leido)
    :else
      (generar-mensaje-error :warning-paren)
  )))
  ([lectura-anterior] (
    let [cadena-nueva (st/join " " (list lectura-anterior (read-line)))] (
      cond
        (<= 1 (verificar-parentesis cadena-nueva)) (leer-entrada cadena-nueva)
        (= 0 (verificar-parentesis cadena-nueva)) (imprimir cadena-nueva)
      :else
        (generar-mensaje-error :warning-paren)
  )))
)

; (println (leer-entrada))
...
(defn actualizar-amb [amb clave valor] (
  cond
    (and (list? valor) (error? valor)) amb
  :else (
    let [nuevo-clave-valor (list clave valor)] (
      let [clave-existe (buscar clave amb)] (
        cond
          (error? clave-existe) ; clave no existe
            (fnc-append (list amb nuevo-clave-valor)) ; agrego al ambiente
        :else
          (apply concat (let [splitList (partition 2 amb)] (
            for [el splitList] (
              cond
                (igual? (first el) (first nuevo-clave-valor)) nuevo-clave-valor
              :else
                el
            )
          )
  ...
  cond
    (symbol? var) (
      cond
        (even? (count amb)) (
          let [valorLeido (
            let [splitList (partition 2 amb)] (
              keep identity (for [el splitList] (
                cond
                  (igual? (first el) var) (second el)
              )
            )
          ))] (
            cond
              (empty? valorLeido) (generar-mensaje-error :unbound-variable var)
            :else
              (list (first valorLeido) amb)
          )
        )
      )
  :else
...
(defn error? [lista] (
  cond
    (seq? lista) (
      cond
        (= (symbol ";ERROR:") (first lista)) true
        (= (symbol ";WARNING:") (first lista)) true
      :else
        false
      )
    :else
...
(defn proteger-bool-en-str [cadena] (
  st/replace cadena #"#F|#f|#T|#t" {"#F" "%F" "#f" "%f" "#T" "%T" "#t" "%t"}
))
...
(defn restaurar-bool [cadena] (
  cond
    (seq? cadena) (
      for [item cadena] (
        cond
          (seq? item) (restaurar-bool item)
          (symbol? item) (
            cond
              (igual? item (symbol "%F")) (symbol "#F")
              (igual? item (symbol "%f")) (symbol "#f")
              (igual? item (symbol "%T")) (symbol "#T")
              (igual? item (symbol "%t")) (symbol "#t")
            :else item
          )
        :else item
...
; user=> (igual? 6 "6")
; false
(defn igual? [arg1 arg2]
  (cond
    (= arg1 arg2) true
    (and (string? arg1) (string? arg2)) (= (lower-case arg1) (lower-case arg2))
    (not (or (number? arg1) (number? arg2))) (= (lower-case (str arg1)) (lower-case (str arg2)))
  :else
    false
  )
)
...
(defn fnc-append [lista] (
  let [errores-lista (
    keep identity (for [l lista] (
    cond
      (not (seq? l)) (generar-mensaje-error :wrong-type-arg 'append l)
    )
  ))] (
    cond
      (empty? errores-lista) (apply concat lista)
    :else
      (first errores-lista)
  )
))
...
(defn fnc-equal? [lista] (
  ...
  :else
    (let [val (first lista)] (
      cond
        (every? true? (map (fn [listEleme] (igual? val listEleme)) lista)) (symbol "#t")
      :else
      (symbol "#f")
    ))
  )
...
(defn fnc-read [var] (
  cond
    (empty? var) (restaurar-bool (read-string (proteger-bool-en-str (leer-entrada))))
    (= 1 (count var)) (generar-mensaje-error :io-ports-not-implemented 'read)
  :else (
...
(defn fnc-sumar [lista]
  (cond (not (empty? lista))
    (reduce (fn [arg1 arg2]
      (cond
        (not (number? arg1)) (reduced (generar-mensaje-error :wrong-type-arg1 '+ arg1))
        (not (number? arg2)) (reduced (generar-mensaje-error :wrong-type-arg2 '+ arg2))
        :else
          (+ arg1 arg2)
    )) lista)
  :else
...
(defn fnc-restar [lista]
  ...
  :else
    (generar-mensaje-error :wrong-number-args '-)
  )
)
...
(defn fnc-menor [lista]
  (cond
    (not (empty? lista))
      (let [salidaReduce (reduce (fn [arg1 arg2]
        (cond
          (or (false? arg1) (= (symbol "#f") arg1)) (symbol "#f")
          (not (number? arg1)) (reduced (generar-mensaje-error :wrong-type-arg1 '< arg1))
          (not (number? arg2)) (reduced (generar-mensaje-error :wrong-type-arg2 '< arg2))
          :else
            (cond
...
(defn evaluar-escalar [var amb] (
  cond
    (and (symbol? var) (not (number? var))) (
      let [valorLeido (buscar var amb)] (
        cond
          (error? valorLeido) (list (generar-mensaje-error :unbound-variable var) amb)
        :else
          (list (first valorLeido) amb)
      )
    )
...
(defn evaluar-define [val amb] (
  cond
    (= 1 (count val)) (list (generar-mensaje-error :missing-or-extra 'define val) amb)
  :else
    (let [params (second val)] (
      cond
        (not (seq? params)) (
          cond
            (not (= 3 (count val))) (list (generar-mensaje-error :missing-or-extra 'define val) amb)
            (and (symbol? params) (not (number? params))) (list (symbol "#<unspecified>") (actualizar-amb amb params (first (evaluar (last val) amb))))
          :else
            (list (generar-mensaje-error :bad-variable 'define val) amb)
        )
      :else (
        cond
          (empty? params) (list (generar-mensaje-error :missing-or-extra 'define val) amb)
          (or (not (symbol? (first params))) (number? (first params))) (list (generar-mensaje-error :bad-variable 'define val) amb)
        :else
          (list (symbol "#<unspecified>") (actualizar-amb amb (first params) (concat (list 'lambda (drop 1 params)) (drop 2 val))))
      )
    )
  )
))
...
(defn evaluar-if [condicion amb] (
  cond
    (not (or (= 3 (count condicion)) (= 4 (count condicion)))) (list (generar-mensaje-error :missing-or-extra 'if condicion) amb)
  :else (
    let [res-evaluar (evaluar (second condicion) amb)] (
    cond
      (and (= (symbol "#f") (first res-evaluar)) (not (= 4 (count condicion)))) (list (symbol "#<unspecified>") amb)
      (and (= (symbol "#f") (first res-evaluar)) (= 4 (count condicion))) (evaluar (nth condicion 3) amb)
    :else
      (evaluar (nth condicion 2) amb)
  )
)))
...
([condicion amb] (
  cond
    (and (= 1 (count condicion)) (= (symbol "or") (first condicion))) (list (symbol "#f") amb)
  :else
    (evaluar-or (drop 1 condicion) amb 1)
  ))
([condicion amb empezado] (
  cond
    (empty? condicion) (list (symbol "#f") amb)
  :else (
    let [res-evaluar (evaluar (first condicion) amb)] (
      cond
        (or (= (symbol "#f") (first res-evaluar)) (= false (first res-evaluar))) (evaluar-or (drop 1 condicion) amb empezado)
      :else
        res-evaluar
    )
  )
...
(defn evaluar-set! [var amb] (
  cond
    (= 3 (count var)) (
      let [existeEnAmb (buscar (second var) amb)] (
        cond
          (error? existeEnAmb) (list existeEnAmb amb)
        :else (
          cond
            (and (symbol? (second var)) (not (number? (second var)))) (list (symbol "#<unspecified>") (actualizar-amb amb (second var) (first (evaluar (last var) amb))))
          :else (
            list (generar-mensaje-error :bad-variable 'set! var) amb
          )
        )
      )
    )
  :else (
    list (generar-mensaje-error :missing-or-extra 'set! var) amb
  )
))
